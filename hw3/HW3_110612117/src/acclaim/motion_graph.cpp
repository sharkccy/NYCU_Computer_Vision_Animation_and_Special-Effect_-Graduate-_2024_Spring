...
void MotionGraph::constructGraph() {
    ...
    for (int i = 0; i < numNodes - 1; i++) {
        ...
        for (int j = i + 1; j < numNodes; j++) {
            bool isConsecutive = (j == i + 1);
            bool isBelowThreshold = distMatrix[i][j] < 200;
            if (isConsecutive) {
                m_graph[i].addEdgeTo(j, 0.5);
            } else if (isBelowThreshold) {
                double weight = 1.0 / distMatrix[i][j];
        ...
        if (sumWeight > 0.0) {
            for (auto& edge : edges) {
                edge.second /= sumWeight;
                m_graph[i].addEdgeTo(edge.first, edge.second * 0.5);
            }
        }
    }
}
