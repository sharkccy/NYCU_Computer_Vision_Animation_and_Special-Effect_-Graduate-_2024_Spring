...
bool flagActivate, curtainActivate = false;
...
void keyCallback(GLFWwindow* window, int key, int, int action, int) {
  ...
  if (key == GLFW_KEY_ESCAPE) {
    ...
    flagActivate = !flagActivate;
    printf("flagActivate: %d\n", flagActivate);
  } else if (key == GLFW_KEY_C) {
    curtainActivate = !curtainActivate;
    printf("curtainActivate: %d\n", curtainActivate);
  }
}
...
int main() {
  ...
  GLFWwindow* window = context.createWindow("HW1 110612117", 1280, 720, GLFW_OPENGL_CORE_PROFILE);
